...
用于阶段二构造搜索词、阶段三描述风格方向、阶段五编写生图提示词。中英对照，覆盖主流设计风格。每种风格附特征、适用场景、英文搜索词。
...
- **生图提示词**：风格词直接嵌入，如 `minimalist poster design, lots of negative space, black white and one accent color`
- **多风格混搭谨慎**：一次生图提示词里最多混 2 种风格（如 `flat illustration with gradient accents`），过多会冲突
